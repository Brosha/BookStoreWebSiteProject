				...
				<table border="1">
					...
					<c:forEach items="${cart.items}" var="item" varStatus="status">
						<tr>
							...
							<td valign="middle"><img class="book_small"
							...
							<td><input type="hidden" name="bookId"
								value="${item.key.bookId}" /> <input type="hidden"
								name="quantity${status.index+1}" value="${item.value}" size="5" />
								${item.value}</td>
					...
					<div>
						<table class="normal">
							<tr>
								...
								<td><button type="reset" onclick="document.location='${pageContext.request.contextPath}/'">Continue Shopping</button>								
